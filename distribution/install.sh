...
BIN="herdr"
MANIFEST_URL="${HERDR_MANIFEST_URL:-https://github.com/Profreshor/herdr-multi-remote/releases/latest/download/latest.json}"
...
main() {
    ...
    echo "     \`WWWWWW'    Profreshor/herdr-multi-remote"
    echo "      II  II"
    echo ""
    ...
    if [ "$OS" = "Linux" ] && [ "$(uname -o 2>/dev/null || true)" = "Android" ]; then
        err "Android/Termux is not currently supported by fork release binaries. SSH to a supported host instead."
    fi
...
need() {
    if ! command -v "$1" >/dev/null 2>&1; then
        err "requires '$1' — install it first, or download a binary manually from https://github.com/Profreshor/herdr-multi-remote/releases"
    fi
}
