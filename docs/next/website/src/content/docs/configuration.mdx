...
When enabled, `herdr --remote` writes a private temporary SSH config that includes your user and system SSH configs, then adds fallback keepalive values. Herdr-managed commands enforce `ServerAliveInterval 15` and `ServerAliveCountMax 4` so dead transports are detected within a bounded interval. Linux and macOS clients also use a private per-attach OpenSSH control socket to reuse the first authenticated connection; Windows OpenSSH does not. Set `manage_ssh_config = false` to run remote attach through plain `ssh` with your SSH keepalive settings unchanged.

To keep local Herdr and several remote Herdr servers in one client, add named remote tables. `host` accepts the same OpenSSH target or alias as `herdr --remote`; `session` is optional, and `enabled` defaults to `true`.
...
Herdr uses normal OpenSSH authentication and does not store keys or passwords. Provision each machine once with `herdr --remote <host>` if it does not already have a compatible Herdr binary. Fleet connections are non-interactive: unavailable machines remain visible and retry in the background without blocking healthy machines.
