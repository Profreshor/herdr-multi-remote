...
## One client for several machines

Named `[remotes.<id>]` entries connect alongside the local server when you launch ordinary `herdr`:
...
The Spaces sidebar groups workspaces under `LOCAL`, `ANALYTICS`, and `PORTAL-STAGING`. Mouse and keyboard workspace navigation switch the active server automatically. Each server still owns its PTYs, processes, agents, persistence, and raw resource IDs; the client scopes those IDs by server while routing commands.

Remote failures are isolated. A disconnected machine is marked as reconnecting while local and other remote workspaces continue operating. Retries use capped backoff and stay non-interactive, version mismatches are shown per machine, and removing or disabling a remote on config reload cancels its connection attempt. Hidden servers continue sending semantic workspace and notification updates but do not render pane surfaces.

Fleet mode uses your normal OpenSSH configuration and authentication. It never prompts to install software inside the UI; run `herdr --remote <host>` once to provision a machine that has no compatible Herdr endpoint.
...
Remote attach supports Linux, macOS, and Windows clients and hosts. Linux and macOS hosts support x86_64 and aarch64; Windows hosts use the x86_64 package, including under Windows ARM64 emulation. Herdr checks the remote platform, prefers a compatible `herdr` already on the remote `PATH`, then checks common direct, Homebrew, mise, Nix, and Windows standalone install paths. Local and remote versions do not need to match once both support the stable endpoint generation. If no compatible binary exists, interactive runs prompt to install one to `~/.local/bin/herdr` on Unix or the per-user Herdr program directory on Windows; non-interactive runs fail instead of modifying the host. If `~/.local/bin` is not on a Unix host's `PATH`, Herdr warns after install.

By default, `herdr --remote` runs remote setup and the bridge through a temporary SSH config that includes your SSH config and fallback keepalive settings. Herdr-managed commands enforce `ServerAliveInterval 15` and `ServerAliveCountMax 4` so dead transports are detected within a bounded interval. Linux and macOS clients also use a private per-attach control socket for connection reuse; Windows OpenSSH does not. Set `[remote].manage_ssh_config = false` to use plain `ssh` with your SSH keepalive settings unchanged.
...
For local builds or custom binaries, set `HERDR_REMOTE_BINARY` to a local file path before running remote attach. Windows targets require the complete `herdr-windows-x86_64.zip` package so the app-local ConPTY runtime is installed with `herdr.exe`.
