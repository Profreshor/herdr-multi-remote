...
有効な場合、`herdr --remote` はユーザーとシステムの SSH 設定を取り込み、フォールバックのキープアライブ値を加えたプライベートな一時 SSH 設定を書き込みます。Herdr が管理するコマンドでは、切断した通信を一定時間内に検出するため `ServerAliveInterval 15` と `ServerAliveCountMax 4` を強制します。Linux と macOS クライアントは、最初に認証した接続を再利用するため、アタッチごとにプライベートな OpenSSH コントロールソケットも使います。Windows OpenSSH ではこの接続再利用を使いません。SSH のキープアライブ設定を変更せず、通常の `ssh` でリモートアタッチを実行するには `manage_ssh_config = false` を設定してください。
