...
## ひとつのクライアントで複数のマシンを操作

通常の `herdr` を起動すると、名前付きの `[remotes.<id>]` エントリがローカルサーバーと同時に接続されます:
...
Spaces サイドバーでは、ワークスペースが `LOCAL`、`ANALYTICS`、`PORTAL-STAGING` の下にグループ化されます。マウスまたはキーボードでワークスペースを移動すると、アクティブなサーバーも自動的に切り替わります。PTY、プロセス、エージェント、永続化状態、リソース ID は引き続き各サーバーが所有し、クライアントがサーバーごとに ID をスコープしてコマンドをルーティングします。

リモートの障害は分離されます。切断したマシンは再接続中と表示されますが、ローカルとほかのリモートワークスペースはそのまま操作できます。再試行には上限付きバックオフを使い、バージョン不一致はマシン単位で表示されます。設定の再読み込みでリモートを削除または無効化すると、その接続試行はキャンセルされます。非表示のサーバーはワークスペースと通知の意味的な更新を引き続き送信しますが、ペインの画面はレンダリングしません。

フリートモードは通常の OpenSSH 設定と認証を使います。UI 内でソフトウェアのインストールを求めることはありません。互換性のある Herdr エンドポイントがないマシンは、先に一度 `herdr --remote <host>` を実行して準備してください。
...
リモートアタッチは、Linux、macOS、Windows のクライアントとホストをサポートします。Linux と macOS ホストは x86_64 と aarch64 に対応し、Windows ホストは Windows ARM64 エミュレーションを含め x86_64 パッケージを使います。Herdr はリモートのプラットフォームを確認し、リモートの `PATH` 上にある互換性のある `herdr` を優先してから、一般的な直接インストール、Homebrew、mise、Nix、Windows スタンドアロンのインストール先を確認します。両方が安定版エンドポイント世代に対応していれば、ローカルとリモートのバージョンが一致する必要はありません。互換性のあるバイナリがない場合、対話的な実行では Unix の `~/.local/bin/herdr` または Windows のユーザー単位 Herdr プログラムディレクトリへのインストールを提案し、非対話的な実行はホストを変更せずに失敗します。Unix ホストの `PATH` に `~/.local/bin` がない場合、Herdr はインストール後に警告します。

デフォルトでは、`herdr --remote` はユーザーの SSH config とフォールバックのキープアライブ設定を含む一時的な SSH config を通してリモートセットアップとブリッジを実行します。Herdr が管理するコマンドでは、切断した通信を一定時間内に検出するため `ServerAliveInterval 15` と `ServerAliveCountMax 4` を強制します。Linux と macOS クライアントは接続再利用用のプライベートなアタッチ単位コントロールソケットも使いますが、Windows OpenSSH では使いません。SSH のキープアライブ設定を変更せず通常の `ssh` を使うには `[remote].manage_ssh_config = false` を設定してください。
