...
启动普通的 `herdr` 时，命名的 `[remotes.<id>]` 条目会与本地服务器同时连接：
...
Spaces 侧边栏会把工作区分组到 `LOCAL`、`ANALYTICS` 和 `PORTAL-STAGING` 下。使用鼠标或键盘切换工作区时，活动服务器也会自动切换。PTY、进程、智能体、持久化状态和原始资源 ID 仍由各自的服务器拥有；客户端按服务器限定这些 ID，并把命令路由到正确的服务器。

远程故障彼此隔离。断开的机器会显示为正在重新连接，而本地和其他远程工作区仍可继续使用。重试采用有上限的退避，版本不匹配会按机器显示；在重新加载配置时移除或禁用远程会取消其连接尝试。隐藏的服务器会继续发送工作区与通知的语义更新，但不会渲染窗格画面。

舰队模式使用常规的 OpenSSH 配置和认证，不会在 UI 内提示安装软件。对于尚无兼容 Herdr 端点的机器，请先运行一次 `herdr --remote <host>` 完成准备。
...
远程连接支持 Linux、macOS 和 Windows 客户端与主机。Linux 和 macOS 主机支持 x86_64 与 aarch64；Windows 主机使用 x86_64 软件包，包括在 Windows ARM64 模拟环境下。Herdr 会检查远程平台，优先使用远程 `PATH` 上兼容的 `herdr`，然后检查常见的直接安装、Homebrew、mise、Nix 和 Windows 独立安装路径。只要两端都支持稳定端点代际，本地与远程版本无需一致。如果没有兼容的二进制文件，交互式运行会询问是否安装到 Unix 的 `~/.local/bin/herdr` 或 Windows 的每用户 Herdr 程序目录；非交互式运行则直接失败，不会修改主机。如果 `~/.local/bin` 不在 Unix 主机的 `PATH` 上，Herdr 会在安装后发出警告。

默认情况下,`herdr --remote` 通过一个包含你的 SSH 配置和兜底保活设置的临时 SSH 配置运行远程设置和桥接。Herdr 管理的命令会强制使用 `ServerAliveInterval 15` 和 `ServerAliveCountMax 4`,以便在有限时间内检测到失效连接。Linux 和 macOS 客户端还会添加一个私有的单次连接 control socket 来复用连接;Windows OpenSSH 不使用此方式。设置 `[remote].manage_ssh_config = false` 可以使用普通 `ssh`,并保持你的 SSH 保活设置不变。
