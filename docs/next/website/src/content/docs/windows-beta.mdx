...
The target host can run Linux, macOS, or Windows. Herdr uses the installed Windows OpenSSH client and your SSH configuration. Windows OpenSSH does not use Herdr's Unix control-socket reuse, so key authentication through Windows `ssh-agent` is recommended to avoid repeated prompts during remote setup. Provisioning a Windows target installs the complete per-user package, including its app-local ConPTY runtime.
